...
VLLM_API_URL: str = os.getenv("VLLM_API_URL")
METRICS_URL:  str = os.getenv("METRICS_URL")
MODEL_NAME:   str = os.getenv("MODEL_NAME")
MODEL_FOR_TOKENIZER: str = os.getenv("MODEL_FOR_TOKENIZER")
MAKE_PLOT: bool = os.getenv("MAKE_PLOT", "1") != "0"
print("MODEL_FOR_TOKENIZER", MODEL_FOR_TOKENIZER)

# VLLM_API_URL = "http://localhost:8000/v1/completions"  # Use completions endpoint
# METRICS_URL  = "http://localhost:8000/metrics"  # Same port as API
# MODEL_NAME = "Qwen3-VL-30B-A3B-Instruct"  # Model name from vllm serve
# MODEL_FOR_TOKENIZER = "/home/jiaheng/hf_models/Qwen/Qwen3-VL-30B-A3B-Instruct"
...
PROMPT_LENGTHS = [1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000]  # Token counts to test
PROMPT_LENGTHS = [10000, 20000, 30000, 40000]  # Token counts to test
...
NUM_RUNS = 5  # Number of times to repeat each measurement

# ====== Statistics helpers ======
def compute_statistics(measurements):
    """Compute median, mean, and std from multiple measurements.

    Args:
        measurements: List of dicts with metric values

    Returns:
        Dict with median, mean, and std for each metric
    """
    if not measurements:
        return {}
...
def read_metrics():
    try:
        ...
        print(f"[INFO] Make sure vLLM server is running on port 8000")
        raise
...
def create_output_directory():
    """Create a timestamped output directory under benchmark_results."""
    ...
    # Copy the current Python script
    script_path = Path(__file__)
    shutil.copy2(script_path, output_dir / script_path.name)
    print(f"[✓] Copied {script_path.name} to output directory")

    # Try to copy the shell script if it exists
    shell_scripts = [
        script_dir / "start_vllm_with_qwen3vl.sh",
        script_dir / "start_vllm_with_qwen.sh",
        script_dir / "start_vllm.sh",
    ]

    for shell_script in shell_scripts:
        if shell_script.exists():
            shutil.copy2(shell_script, output_dir / shell_script.name)
            print(f"[✓] Copied {shell_script.name} to output directory")
            break
    else:
        print(f"[INFO] No shell script found to copy")
...
def reset_prefix_cache():
    ...
    # Extract base URL (e.g., http://localhost:8000 from http://localhost:8000/v1/completions)
...
def main():
    ...
    if EXPERIMENT_MODE == 'reuse_vs_prefill':
        ...
        raw_measurements = []  # Store all raw measurements

        for f in REUSE_FRACTIONS:
            prompt, N, L = build_prompt_with_reuse(tok, base_text, f)
            print(f"[*] reuse={f:.2f}  LCP={L}/{N}  (running {NUM_RUNS} times)")
            ...
            for run_idx in range(NUM_RUNS):
                ...
                print(f"    Run {run_idx+1}/{NUM_RUNS}: "
                      f"prefill={metrics['prefill']:.4f}s  "
                      f"TTFT={metrics['time_to_first_token']:.4f}s  "
                      f"TPOT={metrics['time_per_output_token']:.4f}s")

            # Compute statistics
            stats = compute_statistics(run_metrics)

            print(f"    MEDIAN: prefill={stats['prefill']:.4f}s  "
                  f"TTFT={stats['time_to_first_token']:.4f}s  "
                  f"TPOT={stats['time_per_output_token']:.4f}s")

            result_entry = {
                "reuse_fraction": f,
                "lcp_tokens": L,
                "prompt_tokens": N,
                "prefill_seconds": stats['prefill'],
                "prefill_seconds_mean": stats['prefill_mean'],
                "prefill_seconds_std": stats['prefill_std'],
                "decode_seconds": stats['decode'],
                "decode_seconds_mean": stats['decode_mean'],
                "decode_seconds_std": stats['decode_std'],
                "time_to_first_token": stats['time_to_first_token'],
                "time_to_first_token_mean": stats['time_to_first_token_mean'],
                "time_to_first_token_std": stats['time_to_first_token_std'],
                "time_per_output_token": stats['time_per_output_token'],
                "time_per_output_token_mean": stats['time_per_output_token_mean'],
                "time_per_output_token_std": stats['time_per_output_token_std'],
            }
        ...
        for prompt_length in PROMPT_LENGTHS:
            ...
            # Now measure with partial reuse - multiple runs
            prompt, N, L = build_prompt_with_reuse(tok, base_text, KV_CACHE_REUSE_RATE)

            print(f"[*] Measuring: prompt_length={prompt_length}  LCP={L}/{N}  reuse={KV_CACHE_REUSE_RATE:.2f}  (running {NUM_RUNS} times)")
    ...
    if EXPERIMENT_MODE == 'reuse_vs_prefill':
        output_csv = output_dir / "kv_reuse_vs_prefill.csv"
        output_csv_raw = output_dir / "kv_reuse_vs_prefill_raw.csv"
        output_png = output_dir / "kv_reuse_vs_prefill.png"
        output_json = output_dir / "token_metrics_reuse.json"
    else:  # prompt_length_vs_tpot
        output_csv = output_dir / f"prompt_length_vs_tpot_reuse{int(KV_CACHE_REUSE_RATE*100)}.csv"
        output_csv_raw = output_dir / f"prompt_length_vs_tpot_reuse{int(KV_CACHE_REUSE_RATE*100)}_raw.csv"
        output_png = output_dir / f"prompt_length_vs_tpot_reuse{int(KV_CACHE_REUSE_RATE*100)}.png"
        output_json = output_dir / f"token_metrics_prompt_length_reuse{int(KV_CACHE_REUSE_RATE*100)}.json"

    # Save aggregated statistics (median) CSV
    import csv
    with open(output_csv, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(results[0].keys()))
        w.writeheader()
        w.writerows(results)
    print(f"[✓] Saved aggregated results (median): {output_csv}")

    # Save raw measurements CSV
    with open(output_csv_raw, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(raw_measurements[0].keys()))
        w.writeheader()
        w.writerows(raw_measurements)
    print(f"[✓] Saved raw measurements: {output_csv_raw}")
    ...
    if MAKE_PLOT:
        try:
            ...
            if EXPERIMENT_MODE == 'reuse_vs_prefill':
                ...
                yerr = [r["prefill_seconds_std"] for r in results]

                plt.figure()
                plt.title(f"vLLM Prefill Time vs. Prefix Reuse (N={NUM_RUNS} runs, median±std)")
                plt.xlabel("Reuse fraction (LCP / N)")
                plt.ylabel("Prefill time (s)")
                plt.errorbar(xs, ys, yerr=yerr, marker="o", capsize=5, capthick=2, label="Median ± Std")
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                ...
                fig.suptitle(f"Prompt Length Impact (KV Cache Reuse: {KV_CACHE_REUSE_RATE:.0%}, N={NUM_RUNS} runs, median±std)")

                xs = [r["prompt_length"] for r in results]

                # Plot 1: Time per output token vs prompt length
                ys = [r["time_per_output_token"] for r in results]
                yerr = [r["time_per_output_token_std"] for r in results]
                axes[0, 0].errorbar(xs, ys, yerr=yerr, marker="o", color='blue', capsize=5, capthick=2)
                ...
                ys = [r["time_to_first_token"] for r in results]
                yerr = [r["time_to_first_token_std"] for r in results]
                axes[0, 1].errorbar(xs, ys, yerr=yerr, marker="o", color='green', capsize=5, capthick=2)
                ...
                ys = [r["prefill_seconds"] for r in results]
                yerr = [r["prefill_seconds_std"] for r in results]
                axes[1, 0].errorbar(xs, ys, yerr=yerr, marker="o", color='orange', capsize=5, capthick=2)
                ...
                ys = [r["decode_seconds"] for r in results]
                yerr = [r["decode_seconds_std"] for r in results]
                axes[1, 1].errorbar(xs, ys, yerr=yerr, marker="o", color='red', capsize=5, capthick=2)
